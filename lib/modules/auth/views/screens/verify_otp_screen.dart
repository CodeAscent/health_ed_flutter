...
class _VerifyOtpScreenState extends State<VerifyOtpScreen> {
  ...
  void resendOtp() {
    isResend = true;
    final loginRequest = LoginRequest(mobile: widget.phoneNumber);
    context
        .read<AuthBloc>()
        .add(AuthLoginRequested(loginRequest: loginRequest));
    setState(() {
      ...
      child: BlocConsumer<AuthBloc, AuthState>(
        listener: (context, state) {
          if (state is AuthOtpVerifyFailure) {
            ...
            customSnackbar(
                state.otpVerifyResponse.message!, ContentType.success);
            if (state.otpVerifyResponse.data!.currentStep == 1 &&
                state.otpVerifyResponse.data!.user!.fullName == null) {
              Get.off(() => SignupScreen());
            } else if (state.otpVerifyResponse.data!.currentStep == 1 &&
                state.otpVerifyResponse.data!.user!.fullName != null) {
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          Icon(
                                            Icons
                                                .timer_outlined, // Choose the desired icon
                                    ...
                                    if (otp.length == 4) {
                                      ...
                                      context.read<AuthBloc>().add(
                                          AuthOtpVerifyRequested(
                                              otpVerifyRequest: verifyRequest));
                                    } else {
                                      customSnackbar(
